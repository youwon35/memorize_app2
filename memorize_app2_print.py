...
2026년 5월 12일, 첫 실행 자동 언어 감지와 다국어 번역 점검을 추가했다.

1. 앱 첫 실행 시 기기 언어를 자동으로 감지하도록 했다.
   - `Intl.DateTimeFormat().resolvedOptions().locale`뿐 아니라 React Native의 `NativeModules`에서 얻을 수 있는 iOS/Android locale 후보도 함께 확인한다.
   - 기기 언어가 한국어면 `ko`, 영어면 `en`, 일본어면 `ja`로 시작한다.
   - 지원하지 않는 언어권은 외국 사용자가 이해하기 쉽도록 영어(`en`)로 fallback되게 했다.

2. 사용자가 앱 안에서 직접 고른 언어는 계속 존중되도록 저장 타이밍을 고쳤다.
   - 기존에는 앱 시작 직후 초기 언어값이 AsyncStorage에 먼저 저장되면서, 저장된 사용자 선택을 덮어쓸 가능성이 있었다.
   - 이제 저장된 언어/테마 값을 먼저 읽은 뒤 `preferencesReady`가 된 후에만 다시 저장한다.
   - 첫 설치 사용자는 자동 감지 언어로 시작하고, 기존 사용자는 앱 정보 탭에서 고른 언어가 유지된다.

3. 튜토리얼도 현재 언어를 그대로 따라가게 확인했다.
   - 튜토리얼 제목, 본문, 대기 문구, 버튼 문구는 모두 `createTranslator(language)`를 통해 표시된다.
   - 따라서 한국어/영어/일본어 사용자는 첫 튜토리얼부터 같은 언어로 안내를 받는다.

4. 영어와 일본어 튜토리얼 문장 일부를 더 자연스럽게 다듬었다.
   - 영어 튜토리얼 예시는 `Hello / Hi`로 맞췄다.
   - 일본어 튜토리얼 예시는 `Hello / こんにちは`로 맞췄다.
   - 영어 탭 이름과 맞지 않던 `App Info` 표현은 실제 탭 이름인 `About`으로 맞췄다.
   - 최근 세션 안내도 실제 버튼 문구인 `Retry missed only`와 일치하도록 수정했다.

5. 번역 누락 여부를 점검했다.
   - 앱에서 실제 호출하는 번역 키 267개를 한국어/영어/일본어 전체에서 검사했고 누락은 0개였다.
   - 번역 파일 전체 키도 세 언어 모두 348개로 일치했다.
   - `App.js`와 `src` 내부에서 `src/i18n.js` 밖에 남아 있는 한국어/일본어 하드코딩 문구가 없는지도 확인했다.
