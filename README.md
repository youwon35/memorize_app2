...
Memora is a mobile memorization app built with Expo React Native. Users can save text pairs such as `(A, B)`, practice them in both directions, and manage their saved cards from a simple bottom-tab layout.

## What is included

- Pair creation with two inputs and a `+` flow for adding more cards
- Local persistence with `AsyncStorage`
- Random quiz mode that can ask `A -> B` or `B -> A`
- Case-insensitive answer checking
- Saved pair edit and delete screen
- App info screen
...
- `App.js`: main mobile UI and app flow
...
6. Add `memora://auth/callback` to the Supabase redirect allow list.

## Android release path

1. Replace `com.memora.app` in `app.json` with your final package name.
...
- When Supabase is configured, Google login is used to sync the memorization pairs across devices.
